...
```{r}
lm <- linear_reg() %>% set_engine("lm")
lm1 <- lm %>% fit(deny ~ black, hmda_one) %>% peek()
```
...
hmda_two <-
  ...
  mutate(deny = case_when(s7 == 1 | s7 == 2 ~ 0, s7 == 3 ~ 1, TRUE ~ NA_real_),
         black = s13 == 3,
         lwage = ifelse(s31a == 0, 0, log(s31a)),
...
```{r}
Y2 <- hmda_two$deny
X2 <- cbind(1, hmda_two$black)
W2 <- cbind(1, hmda_two$lwage, hmda_two$chist, hmda_two$mhist, hmda_two$phist, hmda_two$emp)
```
...
hmda_three <- 
  ...
  transmute(
    # Initial variables
    deny = case_when(s7 == 1 | s7 == 2 ~ 0, s7 == 3 ~ 1, TRUE ~ NA_real_),
    black = s13 == 3,
    lwage = ifelse(s31a == 0, 0, log(s31a)),
    emp = ifelse(s25a > 1000, NA, s25a),
    emp_2 = emp^2,
   ...
   # Marital status of applicant
    ...
    dr = check(s45),
    # Number of separate consumer credit lines on credit reports
    clines = check(s41),
    clines_2 = clines^2,
    # Is male
    male = s15 == 1, 
    ...
    s33 = check(s33),
    # Loan amount (in thousands) / Appraised value (in thousands)
    lr = s6 / s50,
    lr_2 = lr^2,
    # Purchase price (in thousands) / Appraised value (in thousands)
    pr = s33/s50,
    pr_2 = pr^2,
    # Co-applicant sex (not applicable)
    coap = s16 == 4,
    # Years of education
    school = check(school),
    # Number of times application was reviewed by underwriter
    s57 = check(s57),
    # Term of loans (months)
    s48 = check(s48),
    # Special loan application program
    s39 = check(s39),
    # Change in median value of property in a given tract, 1980-1990
    chval = check(chval),
    # Number of units in property purchased
    s20 = check(s20),
    # Total monthly income of coapplicant (in dollars)
    lwage_coap = ifelse(s31c == 0, 0, log(s31c)),
    lwage_coap2 = ifelse(coap == 1, lwage_coap, 0),
    male_coap = s16 == 1,
   # Other variables
    rtdum, s24a, s27a, s53, s55, s56, bd, mi, old, vr, uria, netw, dnotown, dprop,
  ) %>% 
  drop_na()
```
...
```{r}
Y3 <- hmda_two$deny
X3 <- cbind(1, hmda_two$black)
W3 <- 
  hmda_three %>% 
  select(
  lwage, chist, mhist, phist, emp, emp_2, married, dr, clines, male,
  suff, assets, lr, pr, coap, s20, s24a, s27a, s39, s48, s53, s55, s56,
  s57, chval, school, bd, mi, old, vr, uria, netw, dnotown, dprop,
  lwage_coap2, lr_2, pr_2, clines_2, rtdum
  ) %>%
  as.matrix()
...
for(i in 1:K) {
   index_k <- round(runif(length(Y3), min = 1, max = length(Y3)))
